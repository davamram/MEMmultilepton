...
void CombineTwoHypotheses(Permutations* Perm1, int iPerm1, Permutations* Perm2, int iPerm2, Permutations* PermMean){
  ...
  //cout <<"CombineTwoHypotheses iPerm1="<< iPerm1<< " iPerm2="<< iPerm2<<endl;

  if (iPerm1!=-1 && iPerm2!=-1){
    if (Perm1->computeHyp || Perm2->computeHyp){
      CombineHypotheses(*Perm1, *Perm2,
        &((*PermMean).resMEM_avgExl0.weight),
	&((*PermMean).resMEM_avgExl0.weightlog),
        &((*PermMean).resMEM_avgExl0.err),
        &((*PermMean).resMEM_avgExl0.chi2),
        &((*PermMean).resMEM_avgExl0.time),
        &((*PermMean).resMEM_avgIncl0.weight),
        &((*PermMean).resMEM_max.weight),
        &((*PermMean).resMEM_logavgExl0.weight),
        &((*PermMean).resKin_maxKinFit.weight),
        &((*PermMean).resKin_maxKinFit_Int.weight),
        &((*PermMean).resMEM_maxKinFit.weight),
        &((*PermMean).resMEM_maxKinFit_Int.weight),
        &((*PermMean).resMEM_avgExl0_JEC_up.weight),
        &((*PermMean).resMEM_avgExl0_JEC_down.weight),
        &((*PermMean).resMEM_avgExl0_JER_up.weight),
        &((*PermMean).resMEM_avgExl0_JER_down.weight));
    }
  }
  else if (iPerm1!=-1 && iPerm2==-1){
    //PermMean = Perm1;
    PermMean->CopyResults(*Perm1);
  }
  else if (iPerm1==-1 && iPerm2!=-1){
    //PermMean = Perm2;
    PermMean->CopyResults(*Perm2);
  }
