...
#include <iostream> 
#include <iomanip> 
#include "Parameters_dim6top_LO_UFO_ctl2_ggttll.h"

// Initialize static instance
Parameters_dim6top_LO_UFO_ctl2_ggttll * Parameters_dim6top_LO_UFO_ctl2_ggttll::instance = 0; 

// Function to get static instance - only one instance per program
Parameters_dim6top_LO_UFO_ctl2_ggttll * Parameters_dim6top_LO_UFO_ctl2_ggttll::getInstance()
{
  if (instance == 0)
    instance = new Parameters_dim6top_LO_UFO_ctl2_ggttll(); 

  return instance; 
}

void Parameters_dim6top_LO_UFO_ctl2_ggttll::setIndependentParameters(SLHAReader& slha)
{
  // Define "zero"
  zero = 0; 
  ZERO = 0; 
  ...
  mdl_ctl2 = slha.get_block_entry("dim6", 28, -1.700000e+00); 
...
void Parameters_dim6top_LO_UFO_ctl2_ggttll::setIndependentCouplings()
{
  GC_1 = -(mdl_ee * mdl_complexi)/3.; 
  GC_2 = (2. * mdl_ee * mdl_complexi)/3.; 
  GC_3 = -(mdl_ee * mdl_complexi); 
  GC_650 = -(mdl_cw * mdl_ee * mdl_complexi)/(2. * mdl_sw); 
  GC_660 = -(mdl_ee * mdl_complexi * mdl_sw)/(6. * mdl_cw); 
  GC_661 = (mdl_ee * mdl_complexi * mdl_sw)/(2. * mdl_cw); 
  GC_585 = (mdl_ctl2 * mdl_complexi)/mdl_Lambda__exp__2; 
}
void Parameters_dim6top_LO_UFO_ctl2_ggttll::setDependentParameters()
{
  mdl_sqrt__aS = sqrt(aS); 
  G = 2. * mdl_sqrt__aS * sqrt(M_PI); 
  mdl_G__exp__2 = ((G) * (G)); 
}
void Parameters_dim6top_LO_UFO_ctl2_ggttll::setDependentCouplings()
{
  GC_6 = -G; 
  GC_7 = mdl_complexi * G; 
}
...
void Parameters_dim6top_LO_UFO_ctl2_ggttll::printIndependentParameters()
{
  cout <<  "dim6top_LO_UFO_ctl2 model parameters independent of event kinematics:" <<
      endl;
...
void Parameters_dim6top_LO_UFO_ctl2_ggttll::printIndependentCouplings()
{
  cout <<  "dim6top_LO_UFO_ctl2 model couplings independent of event kinematics:" <<
      endl;
  ...
  cout << setw(20) <<  "GC_3 " <<  "= " << setiosflags(ios::scientific) <<
      setw(10) << GC_3 << endl;
  cout << setw(20) <<  "GC_650 " <<  "= " << setiosflags(ios::scientific) <<
      setw(10) << GC_650 << endl;
  cout << setw(20) <<  "GC_660 " <<  "= " << setiosflags(ios::scientific) <<
      setw(10) << GC_660 << endl;
  cout << setw(20) <<  "GC_661 " <<  "= " << setiosflags(ios::scientific) <<
      setw(10) << GC_661 << endl;
  cout << setw(20) <<  "GC_585 " <<  "= " << setiosflags(ios::scientific) <<
      setw(10) << GC_585 << endl;
}
void Parameters_dim6top_LO_UFO_ctl2_ggttll::printDependentParameters()
{
  cout <<  "dim6top_LO_UFO_ctl2 model parameters dependent on event kinematics:" <<
      endl;
...
void Parameters_dim6top_LO_UFO_ctl2_ggttll::printDependentCouplings()
{
  cout <<  "dim6top_LO_UFO_ctl2 model couplings dependent on event kinematics:"
      << endl;
